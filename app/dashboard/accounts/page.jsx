...
function StatusBadge({ status, mustChangePassword }) {
  ...
  return (
    <div className="flex flex-col items-start gap-1.5">
      ...
      {mustChangePassword && (
        <span className="inline-flex rounded-full bg-amber-50 px-2 py-0.5 text-[11px] font-bold text-amber-700 ring-1 ring-amber-100">
          Chưa kích hoạt
        </span>
      )}
    </div>
...
export default function AccountsPage() {
  ...
  const fetchUsers = useCallback(async () => {
    ...
    try {
      ...
      setUsers(nextUsers);
      setTotalElements(data.totalElements ?? nextUsers.length);
      setCurrentPage(data.currentPage ?? DEFAULT_PAGE);
    ...
    <>
      <section className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
        <div>
          ...
          <h1 className="mt-2 text-2xl font-bold tracking-[-0.01em] text-[#191c1e]">Quản lý tài khoản hệ thống</h1>
          <p className="mt-2 max-w-3xl text-sm leading-6 text-[#45474c]">
            Theo dõi và quản trị toàn bộ tài khoản nhân sự, khách thuê và khách tìm hiểu trong hệ thống nhà trọ.
          </p>
        </div>
        {canCreateStaff && (
          <button
        ...
        <section className="grid gap-3">
          {isUsingMock && (
            <InlineAlert tone="info">
              Backend chưa sẵn sàng, trang đang dùng dữ liệu giả lập và vẫn hỗ trợ lọc trực tiếp. Nhéeeeeeeeeeeee
            </InlineAlert>
          )}
            ...
            <tbody>
              {users.map((account) => (
                ...
                >
                  ...
                  <td className="px-5 py-4">
                    <StatusBadge status={account.status} mustChangePassword={account.mustChangePassword} />
                    {account.deletedAt && (
                      <span className="mt-2 inline-flex rounded-full bg-slate-100 px-2 py-0.5 text-[11px] font-bold text-slate-600 ring-1 ring-slate-200">
                        Đã xóa mềm
                      </span>
                    )}
                  </td>
                  ...
                  <td className="px-5 py-4">
                    <span className="inline-flex rounded-full bg-[#f2f4f6] px-2.5 py-1 text-xs font-bold text-[#505f76]">
        ...
        >
          <form id="create-staff-form" onSubmit={submitCreateStaff} className="grid gap-4">
            ...
            <label className="grid gap-2">
              ...
              />
              {staffFormErrors.phone && <span className="text-xs font-bold text-rose-600">{staffFormErrors.phone}</span>}
            </label>
            <label className="grid gap-2">
            ...
            <label className="grid gap-2">
              ...
              {staffFormErrors.role && <span className="text-xs font-bold text-rose-600">{staffFormErrors.role}</span>}
            </label>
          </form>
        </Modal>
      )}
          ...
          <div className="grid gap-4">
            ...
            <label className="grid gap-2">
              ...
              <span className="relative">
                <KeyRound className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-[#6b7280]" />
                <select
                  value={selectedRole}
                  onChange={(event) => {
