...
function StatusBadge({ status }) {
  ...
  return (
    <span
      className={`inline-flex rounded-full border px-3 py-1 text-xs font-bold ${meta.className}`}
    >
      {meta.label}
    </span>
  );
...
function InlineAlert({ tone = "error", children }) {
  ...
  return (
    <div
      className={`flex items-start gap-2 rounded-lg border px-4 py-3 text-sm font-semibold ${className}`}
    >
...
export default function StaffAccountsPage() {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      setMessage(
        "Đã tạo tài khoản quản lý và gửi thông tin đăng nhập qua email.",
      );
  ...
  const handleToggleStatus = async (account) => {
    ...
    try {
      ...
      setError(
        statusError?.message || "Không cập nhật được trạng thái tài khoản.",
      );
    } finally {
      setSaving(false);
    }
