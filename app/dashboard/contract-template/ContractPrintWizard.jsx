...
function PrintLine({ label, value }) {
  return (
    <p>
      {label}: <span className="border-b border-dotted border-slate-500 px-2 font-semibold">{value || ".........."}</span>
    </p>
  );
}
...
export default function ContractPrintWizard({ contract, details, occupants = [], onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-[70] flex items-center justify-center bg-[#091426]/70 p-3 backdrop-blur-sm" onClick={onClose}>
      <section className="max-h-[94vh] w-full max-w-[1160px] overflow-hidden rounded-2xl bg-white shadow-2xl" onClick={(event) => event.stopPropagation()}>
        <header className="relative bg-[#05091d] px-5 py-5 text-white">
          <button type="button" onClick={onClose} className="absolute right-4 top-4 rounded-lg p-2 text-slate-300 hover:bg-white/10 hover:text-white" aria-label="Đóng in hợp đồng">
            <X className="h-5 w-5" />
          </button>
          <p className="text-[11px] font-bold uppercase tracking-[0.24em] text-slate-300">Preview luồng hợp đồng thuê</p>
          <h2 className="mt-3 text-2xl font-extrabold">{form.contractCode || "Chưa có mã hợp đồng"}</h2>
          <div className="mt-3 flex flex-wrap gap-2 text-[11px] font-bold">
            <span className="rounded-full bg-white/10 px-3 py-1">Hợp đồng {contract?.status || "DRAFT"}</span>
            <span className="rounded-full bg-white/10 px-3 py-1">Phòng {form.roomCode || "chưa cập nhật"}</span>
            <span className="rounded-full bg-white/10 px-3 py-1">{contract?.contractFileId ? "Đã có bản ký" : "Chưa có bản ký"}</span>
          </div>
        </header>

        <div className="grid max-h-[calc(94vh-132px)] overflow-y-auto bg-[#f3f6fb] p-4 lg:grid-cols-[230px_minmax(0,1fr)]">
          <aside className="grid content-start gap-3">
            <div className="rounded-xl border border-[#dfe5ef] bg-white p-3">
              <p className="mb-3 text-xs font-extrabold uppercase tracking-[0.08em] text-[#607089]">Các bước xử lý</p>
              {STEP_ITEMS.map((item) => (
                <button
                  key={item.id}
                  type="button"
                ...
                >
                  <span className={`grid h-6 w-6 place-items-center rounded-full ${step === item.id ? "bg-white/15" : "bg-white"}`}>{item.id}</span>
                  {item.label}
                </button>
              ))}
            </div>
            <div className="rounded-xl border border-[#dfe5ef] bg-white p-4 text-xs leading-5 text-[#607089]">
          ...
          <main className="mt-4 rounded-xl border border-[#dfe5ef] bg-white p-4 lg:ml-4 lg:mt-0 xl:p-5">
            {step === 1 && (
              <div>
                ...
                <p className="mt-1 text-sm text-[#607089]">Dữ liệu lấy từ hợp đồng/API; trường thiếu để trống, không dùng mock bên B.</p>
                <div className="mt-5 grid gap-4 lg:grid-cols-2">
                  <section className="rounded-xl border border-[#dfe5ef] p-4">
                    <h4 className="mb-4 inline-flex items-center gap-2 font-extrabold text-[#091426]"><Home className="h-4 w-4" />Thông tin phòng</h4>
                    <div className="grid gap-3">
                      <Field label="Cơ sở" value={form.propertyName} onChange={(value) => updateForm("propertyName", value)} />
                      <Field label="Phòng" value={form.roomCode} onChange={(value) => updateForm("roomCode", value)} />
                      <Field label="Giá thuê/tháng" type="number" value={form.monthlyRent} onChange={(value) => updateForm("monthlyRent", value)} />
                      <Field label="Tiền cọc" type="number" value={form.depositAmount} onChange={(value) => updateForm("depositAmount", value)} />
                      <Field label="Số người ở" type="number" value={form.occupantsCount} onChange={(value) => updateForm("occupantsCount", value)} />
                    </div>
                  </section>
                  <section className="rounded-xl border border-[#dfe5ef] p-4">
                    <h4 className="mb-4 inline-flex items-center gap-2 font-extrabold text-[#091426]"><CalendarDays className="h-4 w-4" />Thông tin hợp đồng</h4>
                    <div className="grid gap-3">
                      <Field label="Mã hợp đồng" value={form.contractCode} onChange={(value) => updateForm("contractCode", value)} />
                      <Field label="Ngày bắt đầu" type="date" value={form.startDate} onChange={(value) => updateForm("startDate", value)} />
                      <Field label="Ngày kết thúc" type="date" value={form.endDate} onChange={(value) => updateForm("endDate", value)} />
                      <Field label="Ngày bắt đầu tính tiền" type="date" value={form.rentStartDate} onChange={(value) => updateForm("rentStartDate", value)} />
                      <Field label="Chu kỳ thanh toán/tháng" type="number" value={form.paymentCycleMonths} onChange={(value) => updateForm("paymentCycleMonths", value)} />
                    </div>
                  </section>
                  <section className="rounded-xl border border-[#dfe5ef] p-4 lg:col-span-2">
                    <h4 className="mb-4 inline-flex items-center gap-2 font-extrabold text-[#091426]"><Users className="h-4 w-4" />Thông tin bên thuê</h4>
                    <div className="grid gap-3 md:grid-cols-2">
                      <Field label="Họ tên" value={form.tenantName} onChange={(value) => updateForm("tenantName", value)} />
                      <Field label="Ngày sinh" type="date" value={form.tenantBirthDate} onChange={(value) => updateForm("tenantBirthDate", value)} />
                      <Field label="Số điện thoại" value={form.tenantPhone} onChange={(value) => updateForm("tenantPhone", value)} />
                      <Field label="Số CCCD" value={form.tenantIdentityNumber} onChange={(value) => updateForm("tenantIdentityNumber", value)} placeholder="Để trống nếu chưa có" />
                      <Field label="Ngày cấp" type="date" value={form.tenantIdentityIssuedDate} onChange={(value) => updateForm("tenantIdentityIssuedDate", value)} />
                      <Field label="Nơi cấp" value={form.tenantIdentityIssuedPlace} onChange={(value) => updateForm("tenantIdentityIssuedPlace", value)} />
                      <Field label="Người thân khẩn cấp" value={form.emergencyName} onChange={(value) => updateForm("emergencyName", value)} />
                      <Field label="SĐT người thân" value={form.emergencyPhone} onChange={(value) => updateForm("emergencyPhone", value)} />
                      <div className="md:col-span-2">
                        <Field label="Địa chỉ thường trú" value={form.tenantAddress} onChange={(value) => updateForm("tenantAddress", value)} />
                      </div>
                    </div>
                  </section>
                </div>
            ...
            {step === 2 && (
              <div>
                ...
                <p className="mt-1 text-sm text-[#607089]">Thông tin này sẽ được fill vào hợp đồng trước khi in cho khách ký.</p>
                <section className="mt-5 rounded-xl border border-[#dfe5ef] p-4">
                  <div className="grid gap-3 md:grid-cols-3">
                    <Field label="Ngày bàn giao" type="date" value={handover.handoverDate} onChange={(value) => updateHandover("handoverDate", value)} />
                    <Field label="Chỉ số điện ban đầu" type="number" value={handover.electricReading} onChange={(value) => updateHandover("electricReading", value)} />
                    <Field label="Chỉ số nước ban đầu" type="number" value={handover.waterReading} onChange={(value) => updateHandover("waterReading", value)} />
                  </div>
                </section>
                <div className="mt-4 overflow-x-auto rounded-xl border border-[#dfe5ef]">
                  <table className="w-full min-w-[820px] text-left text-xs">
                    <thead className="bg-[#f7f9fe] text-[10px] font-extrabold uppercase text-[#6b7280]">
                      <tr><th className="px-3 py-3">STT</th><th className="px-3 py-3">Tên thiết bị</th><th className="px-3 py-3">Đơn vị</th><th className="px-3 py-3">SL</th><th className="px-3 py-3">Hiện trạng</th><th className="px-3 py-3">Ghi chú</th></tr>
                    </thead>
            ...
            {step === 3 && (
              <div>
                ...
                <p className="mt-1 text-sm text-[#607089]">PDF sẽ mở ở tab mới, dùng lệnh in của trình duyệt để xuất PDF.</p>
                <section className="mt-5 overflow-hidden rounded-xl border border-[#cbd5e1] bg-[#eef0f4]">
                  <iframe
                  ...
                  <h4 className="text-center text-lg font-bold uppercase">Hợp đồng thuê phòng trọ</h4>
                  ...
                  <table className="mt-4 w-full border-collapse text-xs">
                    <thead><tr className="[&_th]:border [&_th]:p-2"><th>STT</th><th>Tên thiết bị</th><th>Đơn vị</th><th>SL</th><th>Hiện trạng</th><th>Ghi chú</th></tr></thead>
                    <tbody>
                      {assets.map((asset, index) => (
                        <tr key={`${asset.name}-${index}`} className="[&_td]:border [&_td]:p-2">
                          <td>{index + 1}</td><td>{asset.name}</td><td>{asset.unit}</td><td>{asset.quantity}</td><td>{asset.condition}</td><td>{asset.note}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                ...
                <button type="button" onClick={handlePrint} className="mt-4 inline-flex h-10 items-center justify-center gap-2 rounded-lg bg-[#091426] px-4 text-sm font-extrabold text-white hover:bg-[#16253a]">
