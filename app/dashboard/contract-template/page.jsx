...
import {fetchContractHandover, fetchLatestReadings, uploadFile} from "@/services/contractHandoverService";
...
function getLiquidationChargeTypeLabel(value) {
    return LIQUIDATION_CHARGE_TYPES.find((type) => type.value === value)?.label || "Chi phí phát sinh";
}
...
function getLiquidationTariff(lineType, tariffs = DEFAULT_LIQUIDATION_TARIFFS) {
    const key = lineType === "ELECTRICITY" ? "electricity" : lineType === "WATER" ? "water" : null;
    const fallback = DEFAULT_LIQUIDATION_TARIFFS[lineType] || DEFAULT_UTILITY_TARIFFS[key] || {};
    return normalizeUtilityTariff(tariffs?.[lineType] || tariffs?.[key], fallback);
}

function getLiquidationChargeDefaultDescription(lineType) {
    return DEFAULT_LIQUIDATION_CHARGES.find(([type]) => type === lineType)?.[1] || "Chi phí phát sinh";
}
...
function calculateLiquidationRoomRent(item = {}, liquidationDateValue) {
    const liquidationDate = toDateInputValue(liquidationDateValue || item.liquidationDate || item.expectedVacantDate);
    ...
    const periodStart = new Date(liquidation.getFullYear(), liquidation.getMonth(), 1);
    const rentStartValue = toDateInputValue(item.rentStartDate || item.startDate);
    const rentStart = rentStartValue ? new Date(`${rentStartValue}T00:00:00`) : periodStart;
    const chargeStart = rentStart > periodStart ? rentStart : periodStart;
    const endDateValue = toDateInputValue(item.endDate);
    const endDate = endDateValue ? new Date(`${endDateValue}T00:00:00`) : liquidation;
    const chargeEnd = endDate < liquidation ? endDate : liquidation;
    if (chargeEnd < chargeStart) {
        return 0;
    }
    const daysInMonth = new Date(liquidation.getFullYear(), liquidation.getMonth() + 1, 0).getDate();
...
function applyAutoLiquidationRoomRent(charges = [], item = {}, liquidationDateValue) {
...
function createLiquidationCharge(lineType = "OTHER", description = "", tariffs = DEFAULT_LIQUIDATION_TARIFFS) {
    const meterCharge = isMeterLiquidationCharge(lineType);
    return {
        id: `${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
        lineType,
        description: description || getLiquidationChargeDefaultDescription(lineType),
        previousValue: "0",
        currentValue: "0",
        quantity: meterCharge ? "0" : "1",
        unitPrice: meterCharge ? String(getLiquidationTariff(lineType, tariffs).unitPrice) : "0",
        freeAllowance: meterCharge ? String(getLiquidationTariff(lineType, tariffs).freeAllowance) : "0",
        photoFileId: null,
        proofFile: null,
    };
}

function buildLiquidationCharges(item = {}, tariffs = DEFAULT_LIQUIDATION_TARIFFS) {
    ...
    if (lines.length > 0) {
        return lines.map((line) => ({
            ...
            previousValue: line.previousValue == null ? "0" : String(line.previousValue),
...
function buildLiquidationForm(item = {}, tariffs = DEFAULT_LIQUIDATION_TARIFFS) {
    return {
    ...
    return charges
        .map((charge) => {
            const estimate = isMeterLiquidationCharge(charge.lineType)
                ...
                : null;
            const rawQuantity = estimate ? estimate.billableUsage : Number(charge.quantity || 0);
            const unitPrice = estimate ? estimate.unitPrice : Number(charge.unitPrice || 0);
            const quantity = Number.isFinite(rawQuantity) && rawQuantity > 0 ? Math.ceil(rawQuantity) : 0;
            const description = String(charge.description || getLiquidationChargeDefaultDescription(charge.lineType)).trim();
            return {
                ...
                previousValue: isMeterLiquidationCharge(charge.lineType) ? Number(charge.previousValue || 0) : null,
                currentValue: isMeterLiquidationCharge(charge.lineType) ? Number(charge.currentValue || 0) : null,
                photoFileId: isMeterLiquidationCharge(charge.lineType) ? charge.photoFileId ?? null : null,
            };
        })
        .filter((charge) =>
...
function buildRenewForm(item = {}) {
    ...
    return {
        startDate: toDateInputValue(item.startDate),
        currentEndDate,
        endDate: addYearsMinusOneDay(addDays(currentEndDate, 1), 1) || currentEndDate,
        monthlyRent: item.monthlyRent == null ? "" : String(item.monthlyRent),
        paymentCycleMonths: String(item.paymentCycleMonths || 1),
        depositAmount: item.depositAmount == null ? "0" : String(item.depositAmount),
    };
...
function getWorkflow(item) {
    ...
    if (renewalContract && ["DRAFT", "PENDING_SIGNATURE"].includes(contractStatus)) {
    ...
    if (renewalContract && ["MISSING_FILE", "PENDING_ACTIVATION"].includes(status)) {
...
function StatCard({label, value, tone}) {
    ...
    return (
        <article
            className="rounded-xl border border-[#dfe5ef] bg-white p-4 shadow-[0_8px_22px_rgba(15,23,42,0.04)] xl:p-5">
...
function LiquidationInvoiceLines({lines = []}) {
    ...
    const electricityLines = lines.filter((line) => line.lineType === "ELECTRICITY");
    const waterLines = lines.filter((line) => line.lineType === "WATER");
    const otherLines = lines.filter((line) => !isMeterLiquidationCharge(line.lineType));
    const renderLines = (items, {compactMeter = false} = {}) => (
        <table className="w-full table-auto text-left">
            ...
            <tbody>
            {items.map((line) => (
                <tr key={line.id || `${line.lineType}-${line.description}`} className="border-t border-[#edf1f6]">
                    <td className="px-4 py-3 text-sm font-bold text-[#091426]">{getLiquidationChargeTypeLabel(line.lineType)}</td>
                    <td className="px-4 py-3 text-sm font-semibold text-[#091426]">{line.description || "Chưa có"}</td>
                    {!compactMeter && <td className="px-4 py-3 text-right text-sm font-semibold text-[#091426]">{line.quantity ?? 1}</td>}
                    {!compactMeter && <td className="px-4 py-3 text-right text-sm font-semibold text-[#091426]">{formatOptionalMoney(line.unitPrice)}</td>}
                    <td className="px-4 py-3 text-right text-sm font-extrabold text-[#091426]">{formatMoney(getInvoiceLineAmount(line))}</td>
                </tr>
            ))}
            </tbody>
        </table>
...
function LiquidationProofPreview({file, fileId}) {
    ...
    useEffect(() => {
        ...
        fetchLeaseContractFileObjectUrl(fileId)
            ...
            .catch(() => {
                if (isMounted) setPreview({key: remoteKey, url: "", error: "Không tải được ảnh"});
            });
...
function LiquidationChargeRows({charges = [], onAdd, onChange, onProofChange, onRemove}) {
    ...
    const renderChargeRow = (charge, index) => (
        ...
        >
            ...
            {isMeterLiquidationCharge(charge.lineType) ? (
                <>
                    <label className="grid min-w-0 gap-1.5">
                        <span className="text-[11px] font-bold text-[#58667c]">Chỉ số cũ</span>
                        <input
                            type="number"
                            min="0"
                            step="1"
                            value={charge.previousValue}
                            onChange={(event) => onChange(index, "previousValue", event.target.value)}
                            className="h-10 min-w-0 rounded-lg border border-[#cbd5e1] bg-white px-3 text-sm font-semibold outline-none focus:border-[#091426]"
                        />
                    </label>
                    <label className="grid min-w-0 gap-1.5">
                        <span className="text-[11px] font-bold text-[#58667c]">Chỉ số mới</span>
                        <input
                            type="number"
                            min="0"
                            step="1"
                            value={charge.currentValue}
                            onChange={(event) => onChange(index, "currentValue", event.target.value)}
                            className="h-10 min-w-0 rounded-lg border border-[#cbd5e1] bg-white px-3 text-sm font-semibold outline-none focus:border-[#091426]"
                        />
                    </label>
                </>
            ...
            {isMeterLiquidationCharge(charge.lineType) && (
                <label className="grid min-w-0 gap-1.5">
                    <span className="text-[11px] font-bold text-[#58667c]">Ảnh bằng chứng</span>
                    <span className="inline-flex h-10 min-w-0 cursor-pointer items-center justify-center gap-1.5 rounded-lg border border-[#cbd5e1] bg-white px-3 text-xs font-extrabold text-[#091426] hover:bg-[#f8fafc]">
                        ...
                        <span className="truncate">
                            {charge.proofFile?.name || (charge.photoFileId ? "Đã có ảnh" : "Upload ảnh")}
                        </span>
                    </span>
                    <input
                        type="file"
                        accept="image/*"
                        className="sr-only"
                        onChange={(event) => onProofChange(index, event.target.files?.[0] || null)}
                    />
                    <LiquidationProofPreview file={charge.proofFile} fileId={charge.photoFileId}/>
                </label>
            )}
    ...
    if (!charges.length) {
        return (
            <div className="grid gap-3 rounded-lg border border-dashed border-[#dfe5ef] bg-white p-3 sm:col-span-2">
                <div className="flex items-center justify-between gap-3">
                    <p className="text-xs font-bold uppercase tracking-[0.06em] text-[#58667c]">Chi phí khác</p>
                    <button
                        type="button"
                        onClick={onAdd}
                ...
                <p className="text-sm font-semibold text-[#607089]">Chưa có khoản thanh lý nào.</p>
            </div>
        );
    }
    return (
        <div className="grid gap-3 sm:col-span-2">
            {electricityCharges.length > 0 && (
                <div className="grid gap-3">
                    <p className="text-xs font-bold uppercase tracking-[0.06em] text-[#58667c]">Chốt điện</p>
                    {electricityCharges.map(({charge, index}) => renderChargeRow(charge, index))}
                </div>
            )}
            {waterCharges.length > 0 && (
                <div className="grid gap-3">
                    <p className="text-xs font-bold uppercase tracking-[0.06em] text-[#58667c]">Chốt nước</p>
                    {waterCharges.map(({charge, index}) => renderChargeRow(charge, index))}
                </div>
            )}
            <div className="grid gap-3">
                <div className="flex items-center justify-between gap-3">
                ...
                {otherCharges.length > 0 ? (
                    otherCharges.map(({charge, index}) => renderChargeRow(charge, index))
                ) : (
                    <p className="rounded-lg border border-dashed border-[#dfe5ef] bg-white px-4 py-3 text-sm font-semibold text-[#607089]">
...
export default function ContractTemplatePage() {
    ...
    const [liquidationTariffs, setLiquidationTariffs] = useState(DEFAULT_LIQUIDATION_TARIFFS);
    ...
    const mergedSelected = useMemo(() => {
        ...
        return {
            ...
            propertyId: getContractPropertyId(details) ?? getContractPropertyId(selected),
            ...
            liquidationDate: details.liquidationDate ?? selected.liquidationDate ?? null,
            liquidationReason: details.liquidationReason ?? selected.liquidationReason ?? null,
            liquidationDepositAmount: details.liquidationDepositAmount ?? selected.liquidationDepositAmount ?? null,
            liquidationDepositDeductionAmount: details.liquidationDepositDeductionAmount ?? selected.liquidationDepositDeductionAmount ?? null,
            liquidationDepositDeductionReason: details.liquidationDepositDeductionReason ?? selected.liquidationDepositDeductionReason ?? null,
            liquidationDepositRefundAmount: details.liquidationDepositRefundAmount ?? selected.liquidationDepositRefundAmount ?? null,
            liquidationFinalInvoiceId: details.liquidationFinalInvoiceId ?? selected.liquidationFinalInvoiceId ?? null,
            liquidationFinalInvoiceCode: details.liquidationFinalInvoiceCode ?? selected.liquidationFinalInvoiceCode ?? null,
            liquidationFinalInvoiceStatus: details.liquidationFinalInvoiceStatus ?? selected.liquidationFinalInvoiceStatus ?? null,
            liquidationFinalInvoiceSubtotalAmount: details.liquidationFinalInvoiceSubtotalAmount ?? selected.liquidationFinalInvoiceSubtotalAmount ?? null,
            liquidationFinalInvoiceDiscountAmount: details.liquidationFinalInvoiceDiscountAmount ?? selected.liquidationFinalInvoiceDiscountAmount ?? null,
            liquidationFinalInvoiceTotalAmount: details.liquidationFinalInvoiceTotalAmount ?? selected.liquidationFinalInvoiceTotalAmount ?? null,
            liquidationFinalInvoiceRemainingAmount: details.liquidationFinalInvoiceRemainingAmount ?? selected.liquidationFinalInvoiceRemainingAmount ?? null,
            liquidationFinalInvoiceLines:
                Array.isArray(details.liquidationFinalInvoiceLines) && details.liquidationFinalInvoiceLines.length > 0
                    ? details.liquidationFinalInvoiceLines
                    : selected.liquidationFinalInvoiceLines ?? [],
            liquidationSignedFileId: details.liquidationSignedFileId ?? selected.liquidationSignedFileId ?? null,
            liquidationStatus: details.liquidationStatus ?? selected.liquidationStatus ?? null,
            liquidationCreatedAt: details.liquidationCreatedAt ?? selected.liquidationCreatedAt ?? null,
            liquidationDepositRefundRequestId: details.liquidationDepositRefundRequestId ?? selected.liquidationDepositRefundRequestId ?? null,
            liquidationDepositRefundExpenseId: details.liquidationDepositRefundExpenseId ?? selected.liquidationDepositRefundExpenseId ?? null,
            liquidationDepositRefundExpenseRequestId: details.liquidationDepositRefundExpenseRequestId ?? selected.liquidationDepositRefundExpenseRequestId ?? null,
            liquidationDepositRefundStatus: details.liquidationDepositRefundStatus ?? selected.liquidationDepositRefundStatus ?? null,
            liquidationDepositRefundProofFileId: details.liquidationDepositRefundProofFileId ?? selected.liquidationDepositRefundProofFileId ?? null,
            liquidationDepositRefundedAmount: details.liquidationDepositRefundedAmount ?? selected.liquidationDepositRefundedAmount ?? null,
            liquidationDepositRefundedAt: details.liquidationDepositRefundedAt ?? selected.liquidationDepositRefundedAt ?? null,
            liquidationDepositRefundTransactionRef: details.liquidationDepositRefundTransactionRef ?? selected.liquidationDepositRefundTransactionRef ?? null,
        };
    }, [details, selected]);
    ...
    const liquidationDraftDeposit = Number(
        mergedSelected?.liquidationDepositAmount ?? mergedSelected?.depositAmount ?? 0,
    );
    const safeLiquidationDraftDeposit = Number.isFinite(liquidationDraftDeposit) ? liquidationDraftDeposit : 0;
    const liquidationDraftRemainingDeposit = Math.max(0, safeLiquidationDraftDeposit);
    const liquidationDraftRemainingPayable = Math.max(0, liquidationDraftSubtotal);
    ...
    async function handleFileSelected(event) {
        ...
        try {
            ...
            setSelected((current) => {
                ...
                return {
                    ...merged,
                    signedFileId: updated.signedFileId ?? refreshedItem?.signedFileId ?? current?.signedFileId ?? merged.signedFileId,
                    signedFileName: updated.signedFileName ?? refreshedItem?.signedFileName ?? current?.signedFileName ?? merged.signedFileName,
            ...
            if (uploadedContractId) {
                ...
                const mergedDetails = {
                    ...refreshedDetails,
                    signedFileId: refreshedDetails.signedFileId ?? updated.signedFileId ?? refreshedItem?.signedFileId,
                    signedFileName: refreshedDetails.signedFileName ?? updated.signedFileName ?? refreshedItem?.signedFileName,
                    signedFileUploadedAt:
    ...
    async function handleConfirmTransferDocumentSigned(item) {
        ...
        setError("");
        try {
            await signTransferContractDocument(item.transferRequestId, item.leaseContractId);
            const refreshedContracts = await loadContracts();
            const refreshedItem = refreshedContracts.find(
                (contract) => String(contract.leaseContractId) === String(item.leaseContractId),
            );
    ...
    async function openLiquidationEditor(item) {
        ...
        if (!propertyId) {
            setLiquidationError("Không xác định được cơ sở để tải đơn giá điện nước.");
        ...
        try {
            ...
            tariffs = {
                ELECTRICITY: normalizeUtilityTariff(settings?.electricity, DEFAULT_LIQUIDATION_TARIFFS.ELECTRICITY),
                WATER: normalizeUtilityTariff(settings?.water, DEFAULT_LIQUIDATION_TARIFFS.WATER),
            };
        } catch (err) {
            setLiquidationError(err?.message || "Không tải được đơn giá điện nước của cơ sở.");
            toast.error(err?.message || "Không tải được đơn giá điện nước của cơ sở.");
            return;
        }
        ...
        try {
            ...
            setLiquidationForm((current) => ({
                ...
                charges: (current.charges || []).map((charge) => {
                    ...
                    const key = charge.lineType === "ELECTRICITY" ? "electricity" : "water";
                    ...
                    return {
                        ...
                        unitPrice: String(getLiquidationTariff(charge.lineType, tariffs).unitPrice),
                        freeAllowance: String(getLiquidationTariff(charge.lineType, tariffs).freeAllowance),
                    };
                }),
            }));
        } catch (err) {
    ...
    async function handleLiquidate(item) {
        ...
        if (!item.liquidationFinalInvoiceId) {
            ...
            setLiquidationError("Vui lòng nhập và lưu hồ sơ thanh lý trước khi hoàn tất.");
            return;
        }
        ...
        try {
            ...
            if (!payload) return;
            const updated = await liquidateLeaseContract(item.leaseContractId, payload);
        ...
        setLiquidationForm((current) => ({
            ...
            charges: (current.charges || []).map((charge, chargeIndex) => {
                ...
                if (field === "lineType") {
                    ...
                    next.unitPrice = meterCharge ? String(getLiquidationTariff(value, liquidationTariffs).unitPrice) : next.unitPrice || "0";
                    next.freeAllowance = meterCharge ? String(getLiquidationTariff(value, liquidationTariffs).freeAllowance) : "0";
                    ...
                    if (value === "ROOM_RENT") {
                        next.description = getLiquidationRoomRentDescription(current.liquidationDate);
                        next.quantity = "1";
                        next.unitPrice = String(calculateLiquidationRoomRent(mergedSelected || {}, current.liquidationDate));
                    }
                }
                return next;
            }),
    ...
    async function buildLiquidationPayload({uploadPhotos = false} = {}) {
        ...
        for (const charge of liquidationForm.charges || []) {
            ...
            if (
                ...
                setLiquidationError("Vui lòng kiểm tra số lượng và đơn giá các khoản thanh lý.");
                return null;
            }
        }
        if (uploadPhotos) {
    ...
    async function handleSaveLiquidationDraft(item) {
        ...
        try {
            const payload = await buildLiquidationPayload({uploadPhotos: true});
            if (!payload) return;
            const updated = await updateLeaseContractLiquidationDraft(item.leaseContractId, payload);
            await refreshSelectedContract(updated);
            setIsEditingLiquidation(false);
    ...
    async function handleRenewContract() {
        ...
        const validationErrors = {};
        const startDate = renewForm.startDate || toDateInputValue(mergedSelected.startDate);
        const currentEndDate = renewForm.currentEndDate || toDateInputValue(mergedSelected.endDate);
        ...
        if (!renewForm.endDate) {
            ...
            validationErrors.endDate = "Ngày kết thúc sau gia hạn phải sau ngày kết thúc hiện tại.";
        }
        if (!Number.isFinite(monthlyRent) || monthlyRent <= 0) {
        ...
        <div className="w-full min-w-0 flex flex-col gap-6 text-[#091426] text-[13px] xl:text-sm">
            ...
            <section
                ...
                <div className="grid w-full grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
                    ...
                    <label className="relative">
                        <FileCheck2
                            className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-[#8a98af]"/>
                        <select
                            value={fileFilter}
                            onChange={(event) => {
              ...
              <span className="inline-flex min-w-0 items-center gap-2">
                ...
                {timePopoverOpen ? "Thu gọn" : "Mở"}
                        ...
                        {timePopoverOpen && (
                            <div
                                className="absolute left-0 top-[calc(100%+0.5rem)] z-40 w-[320px] max-w-[calc(100vw-2rem)] rounded-xl border border-[#dfe5ef] bg-white p-3 shadow-[0_18px_40px_rgba(15,23,42,0.16)] dark:border-white/10 dark:bg-[#0f172a]">
                                <div className="grid grid-cols-[120px_1fr] gap-3">
                                    <div
                                        className="flex flex-col gap-1 border-r border-[#edf1f6] pr-3 dark:border-white/10">
                                        <button
                                            type="button"
            ...
            {error && (
                <div
                    className="rounded-xl border border-red-200 dark:border-rose-500/20 bg-red-50 dark:bg-rose-500/10 px-4 py-3 text-sm font-bold text-red-700 dark:text-rose-300">
                    {error}
                </div>
            )}
            ...
            {actionMessage && (
                <div
                    className="rounded-xl border border-emerald-200 dark:border-emerald-500/20 bg-emerald-50 dark:bg-emerald-500/10 px-4 py-3 text-sm font-bold text-emerald-700 dark:text-emerald-300">
                    {actionMessage}
                </div>
            )}

            <section
                className="overflow-hidden rounded-xl border border-[#dfe5ef] bg-white shadow-[0_8px_22px_rgba(15,23,42,0.04)]">
                <header className="border-b border-[#dfe5ef] px-5 py-5 xl:px-8 xl:py-7">
                    <h2 className="text-xl font-extrabold tracking-[-0.01em] text-[#091426] xl:text-2xl">Danh sách hợp
                        đồng</h2>
                ...
                <div
                    className="dashboard-table contract-management-table"
                >
                    <table
                        className="table-auto text-left text-[12px] xl:text-sm [&_td]:px-2 [&_td]:py-4 xl:[&_td]:px-2.5 xl:[&_td]:py-4 [&_th]:px-2 [&_th]:py-3 xl:[&_th]:px-2.5 xl:[&_th]:py-3"
                    >
                        <colgroup>
                            <col style={{width: "18%"}}/>
                            <col style={{width: "7%"}}/>
                        ...
                        <thead
                            className="bg-[#f7f9fe] dark:bg-white/5 text-[10px] font-extrabold uppercase tracking-[0.03em] text-slate-500 dark:text-slate-400 xl:text-xs">
                        <tr>
                            ...
                            <th>Giá thuê</th>
                            <th>Trạng thái</th>
                            <th className="contract-management-table__action !px-2 text-center xl:!px-2.5">Thao tác</th>
                        </tr>
                        </thead>
                        ...
                        {!loading &&
                                    ...
                                    <td data-label="Mã HĐ" className="!pl-5 align-middle xl:!pl-6">
                                        ...
                                        <div className="mt-2 flex flex-wrap items-center gap-1.5">
                                            {getWorkflow(item) === "RENEWED" && (
                                                <span
                                                    className="rounded-full border border-blue-200 dark:border-blue-500/20 bg-blue-50 dark:bg-blue-500/10 px-2 py-0.5 text-[10px] font-extrabold text-blue-700 dark:text-blue-300">
                            Hợp đồng cũ
                          </span>
                                            )}
                            ...
                            Hợp đồng hiện tại
                                        ...
                                        {item.previousContractId && (
                                            ...
                                            >
                                                Hợp đồng
                                                trước: {item.previousContractCode || `#${item.previousContractId}`}
                                            </button>
                                        )}
                                    </td>
                        ...
                        <Home className="h-3.5 w-3.5 shrink-0 text-slate-400 dark:text-slate-500 xl:h-4 xl:w-4"/>
                          {item.roomCode || "-"}
                                    ...
                                    <td data-label="Số người" className="align-middle">
                                        <p className="font-extrabold leading-5 text-[#091426]">{getOccupantsCount(item, null)}</p>
                                    </td>
                                    ...
                                    <td data-label="Thao tác"
                                        ...
                                        <div className="flex items-center justify-center">
                                            ...
                                            >
                                                ...
                                                {needsActivationFlow(item) ? "Kích hoạt hợp đồng" : "Xem chi tiết"}
                                            </button>
                                        </div>
                                    </td>
                                </tr>
            ...
            {cleanupModalOpen && (
              ...
              <span
                  className="flex h-11 w-11 shrink-0 items-center justify-center rounded-lg bg-orange-50 text-orange-700 dark:bg-orange-500/10 dark:text-orange-300">
                <AlertTriangle className="h-5 w-5"/>
                        ...
                        <div
                            className="mt-6 rounded-lg border border-orange-200 bg-orange-50 px-4 py-3 text-sm font-semibold text-orange-700 dark:border-orange-500/20 dark:bg-orange-500/10 dark:text-orange-300">
            ...
            {mergedSelected && (
                <div
                    ...
                    <section id="contract-detail-dialog"
                        ...
                        <div className="grid gap-4 px-5 py-5 xl:gap-5 xl:px-7 lg:grid-cols-2">
                            {getTransferContractNotice(mergedSelected) && (
                                <div
                                    className="lg:col-span-2 mt-4 flex items-start gap-3 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm font-semibold leading-6 text-amber-800">
                            ...
                            {isRoomTransferManagedContract(mergedSelected) && !isTransferExecuted(mergedSelected) && (
                                <DetailCard title="Xử lý hợp đồng chuyển phòng" icon={ArrowRightLeft}
                                    ...
                                    <div className="mt-5 grid gap-4 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-end">
                                        <div className="grid gap-3 sm:grid-cols-3">
                                            ...
                                            <InfoValue
                                                label="File đã ký"
                                                value={getLeaseSignedFileId(mergedSelected) ? selectedSignedLeaseContractFilename : "Chưa upload"}
                                            />
                                        </div>
                                        <div className="flex flex-wrap justify-end gap-2">
                                            ...
                                            {isTransferSigningStatus(mergedSelected.transferStatus) && (
                                                <>
                                                    ...
                                                    <button
                                                        type="button"
                                                        onClick={() => handleConfirmTransferDocumentSigned(mergedSelected)}
                                                        disabled={isBusy || !getLeaseSignedFileId(mergedSelected) || mergedSelected.status === "SIGNED"}
                                                    ...
                                                    >
                                                        {actionLoading === `transfer-contract-sign-${mergedSelected.leaseContractId}` ? (
                                                            <Loader2 className="h-4 w-4 animate-spin"/>
                                                        ) : (
                                                            <CheckCircle2 className="h-4 w-4"/>
                                                        )}
                                                        {mergedSelected.status === "SIGNED" ? "Đã xác nhận ký" : "Xác nhận hợp đồng này"}
                                                    </button>
                                                </>
                                            )}
                                        </div>
                                    ...
                                    {!isTransferSigningStatus(mergedSelected.transferStatus) &&
                                        mergedSelected.transferStatus !== "WAITING_CONTRACT_CONFIRMATION" && (
                                            <p className="mt-4 rounded-lg border border-[#dfe5ef] bg-white px-4 py-3 text-sm font-semibold text-[#607089]">
                                                Trạng thái chuyển phòng hiện
                                                tại: {getTransferContractStatusLabel(mergedSelected)}. Tất cả
                                                thao tác xử lý đều làm ngay tại khối này.
                                            </p>
                                        )}
                                </DetailCard>
                            )}
                            {needsActivationFlow(mergedSelected) ? (
                                ...
                                <>
                                    {getWorkflow(mergedSelected) === "EXPIRED" && (
                                        <div
                                            className="rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm font-bold text-red-700 lg:col-span-2">
                                            Hợp đồng đã hết hạn. Vui lòng gia hạn hoặc thanh lý.
                                        </div>
                                    )}
                                    {getWorkflow(mergedSelected) === "RENEWED" && mergedSelected.renewedContractId && (
                                        <div
                                            className="flex flex-col gap-3 rounded-xl border border-blue-200 bg-blue-50 px-4 py-3 text-sm font-bold text-blue-800 lg:col-span-2 sm:flex-row sm:items-center sm:justify-between">
                  <span>
                    Hợp đồng đã được gia hạn sang {mergedSelected.renewedContractCode || `#${mergedSelected.renewedContractId}`}.
                                    ...
                                    <DetailCard title="Thông tin phòng" icon={Home}>
                                        {isEditingTerms ? (
                                            <div className="mt-5 grid gap-4 sm:grid-cols-2 xl:gap-5">
                                                <InfoValue label="Cơ sở"
                                                           value={mergedSelected.propertyName || mergedSelected.property?.name}/>
                                                <InfoValue label="Phòng"
                                                           value={mergedSelected.roomCode || mergedSelected.room?.roomCode}/>
                                                <label className="grid min-w-0 gap-1.5">
                                                    <span
                                                        className="text-xs font-bold text-[#58667c]">Giá thuê/tháng *</span>
                                                    <input
                                                        type="number"
                                                        min="1"
                                                        step="1000"
                          ...
                          {termsFieldErrors.depositAmount}
                                            ...
                                            <div className="mt-5 grid gap-4 sm:grid-cols-2 xl:gap-5">
                                                ...
                                                <InfoValue label="Phòng"
                                                           value={mergedSelected.roomCode || mergedSelected.room?.roomCode}/>
                                                <InfoValue label="Giá thuê/tháng"
                                                           value={formatOptionalMoney(mergedSelected.monthlyRent)}/>
                                                <InfoValue
                                                    label="Số tiền đóng mỗi kỳ"
                                                    value={formatOptionalMoney(getAmountPerPeriod(mergedSelected))}
                                                />
                                                <InfoValue label="Tiền cọc"
                                                           value={formatOptionalMoney(mergedSelected.depositAmount)}/>
                                                <InfoValue
                                                    label="Số người"
                                                    value={`${getOccupantsCount(mergedSelected, details)} người`}
                                                />
                                    ...
                                    <DetailCard
                                        ...
                                        action={
                                            ...
                                            !["LIQUIDATED", "EXPIRED", "CANCELLED", "RENEWED"].includes(getWorkflow(mergedSelected)) ? (
                                                ...
                                                >
                                                    {isEditingTerms ? <X className="h-3.5 w-3.5"/> :
                                                ...
                                                <div className="grid gap-3 sm:grid-cols-2 xl:gap-4">
                                                    ...
                                                    <label className="grid min-w-0 gap-1.5">
                                                        ...
                                                        {termsFieldErrors.startDate && (
                                                            <span
                                                                className="text-xs font-semibold leading-4 text-red-600">
                            {termsFieldErrors.startDate}
                          </span>
                                                        )}
                                                    </label>
                            ...
                            {termsFieldErrors.paymentCycleMonths}
                                                    ...
                                                    <InfoValue label="Ngày bắt đầu tính tiền"
                                                    ...
                                                    <InfoValue
                                                        label="File hợp đồng"
                                                        value={getLeaseSignedFileId(mergedSelected) ? selectedSignedLeaseContractFilename : "Chưa có"}
                                                    />
                                                </div>

                                                <div
                                                    className="mt-4 rounded-lg border border-blue-100 bg-blue-50 px-3 py-2.5 text-xs leading-5 text-blue-800">
                                                    Ngày bắt đầu tính tiền thực
                                                    tế: <strong>{formatDate(previewRentStartDate)}</strong>.
                                                ...
                                                {shortThreeMonthCycle && (
                                                    <div
                                                        className="mt-3 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2.5 text-xs font-semibold leading-5 text-amber-800">
                                                        Thời hạn hợp đồng còn dưới 3 tháng nhưng đang chọn chu kỳ 3
                                                        tháng/lần. Hệ thống vẫn cho lưu, vui lòng kiểm tra lại lịch thu
                                                        tiền.
                                                    </div>
                                                )}
                                            ...
                                            action={
                                                canUseLiquidationActions && mergedSelected.liquidationStatus !== "CONFIRMED" ? (
                                                    <button
                                                        ...
                                                        className="inline-flex h-9 shrink-0 items-center justify-center gap-1.5 rounded-lg border border-[#cbd5e1] bg-white px-3 text-xs font-extrabold text-[#091426] hover:bg-[#f8fafc] disabled:opacity-60"
                                                    >
                                                        {isEditingLiquidation ? <X className="h-3.5 w-3.5"/> :
                                                            <Pencil className="h-3.5 w-3.5"/>}
                                        ...
                                        >
                                            <div className="mt-5 grid gap-4 sm:grid-cols-2 xl:gap-5">
                                                {isEditingLiquidation ? (
                                                    <>
                                                        ...
                                                        />
                                                        <div
                                                            className="grid gap-3 rounded-lg border border-[#dfe5ef] bg-white p-3 sm:col-span-2 sm:grid-cols-4">
                                                            <InfoValue label="Tổng phí chốt"
                                                                       value={formatMoney(liquidationDraftSubtotal)}/>
                                                            <InfoValue label="Khách còn phải trả"
                                                                        value={formatMoney(liquidationDraftRemainingPayable)}/>
                                                            <InfoValue label="Cọc phải hoàn"
                                                                        value={formatMoney(liquidationDraftRemainingDeposit)}/>
                                                        </div>
                                                    ...
                                                    <>
                                                        <InfoValue label="Trạng thái hồ sơ"
                                                                   value={mergedSelected.liquidationStatus === "CONFIRMED" ? "Đã xác nhận" : "Bản nháp"}/>
                                                        <InfoValue label="Ngày thanh lý"
                                                                   value={formatDate(mergedSelected.liquidationDate || mergedSelected.expectedVacantDate)}/>
                                                        <InfoValue label="Cọc đã thanh toán"
                                                                   value={formatOptionalMoney(mergedSelected.liquidationDepositAmount ?? mergedSelected.depositAmount)}/>
                                                        <InfoValue label="Hoàn trả cọc"
                                                                   value={formatOptionalMoney(mergedSelected.liquidationDepositRefundAmount ?? mergedSelected.depositAmount)}/>
                                                        <InfoValue label="Trạng thái hoàn cọc"
                                                                   value={getDepositRefundStatusLabel(mergedSelected.liquidationDepositRefundStatus)}/>
                                                        <InfoValue label="Yêu cầu duyệt hoàn cọc"
                                                                   value={mergedSelected.liquidationDepositRefundExpenseRequestId ? `#${mergedSelected.liquidationDepositRefundExpenseRequestId}` : "Chưa có"}/>
                                                        <InfoValue label="Đã ghi nhận hoàn"
                                                                   value={formatOptionalMoney(mergedSelected.liquidationDepositRefundedAmount)}/>
                                                        <InfoValue label="Mã giao dịch hoàn"
                                                                   value={mergedSelected.liquidationDepositRefundTransactionRef}/>
                                                        <InfoValue label="Hóa đơn chốt"
                                                                   value={mergedSelected.liquidationFinalInvoiceCode || (mergedSelected.liquidationFinalInvoiceId ? `#${mergedSelected.liquidationFinalInvoiceId}` : "Chưa có")}/>
                                                        <InfoValue label="Trạng thái hóa đơn"
                                                                   value={mergedSelected.liquidationFinalInvoiceStatus || "Chưa có"}/>
                                                        <InfoValue label="Tổng phí chốt"
                                                                   value={formatOptionalMoney(mergedSelected.liquidationFinalInvoiceSubtotalAmount)}/>
                                                        <InfoValue label="Khách còn phải trả"
                                                                   value={formatOptionalMoney(mergedSelected.liquidationFinalInvoiceRemainingAmount)}/>
                                                        <InfoValue label="Lý do thanh lý"
                                                                   value={mergedSelected.liquidationReason}/>
                                                        <div className="sm:col-span-2">
                                                            <p className="mb-2 text-xs font-bold uppercase tracking-[0.06em] text-[#58667c]">Chi
                                                                tiết khoản thanh lý</p>
                                                            <LiquidationInvoiceLines
                                                                lines={mergedSelected.liquidationFinalInvoiceLines || []}/>
                                                        </div>
                                                        {Number(mergedSelected.liquidationDepositRefundAmount || 0) > 0 &&
                                                            mergedSelected.liquidationDepositRefundStatus !== "TENANT_CONFIRMED" && (
                                                                <p className="rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-sm font-semibold text-amber-800 sm:col-span-2">
                                                                    Cần hoàn cọc qua yêu cầu chi, upload minh chứng và chờ khách thuê xác nhận trước khi hoàn tất thanh lý.
                                                                </p>
                                                            )}
                                                    </>
                                                )}
                                            ...
                                            {!canUseLiquidationActions && mergedSelected.liquidationStatus !== "CONFIRMED" && (
                                                <p className="mt-4 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-sm font-semibold text-amber-800">
                                                    Chỉ chủ trọ được cập nhật hồ sơ và hoàn tất thanh lý hợp đồng.
                                                </p>
                                            )}
                                            {liquidationError && (
                                            ...
                                            {canUseLiquidationActions && mergedSelected.liquidationStatus !== "CONFIRMED" && (
                                                <div className="mt-5 flex flex-wrap justify-end gap-2">
                                                    ...
                                                    {!isEditingLiquidation && mergedSelected.liquidationFinalInvoiceId && (
                                                        <button
                                                            type="button"
                                    ...
                                    <DetailCard title="Người ở trong hợp đồng" icon={Users} className="lg:col-span-2">
                                        <div
                                            ...
                                            <table className="w-full table-auto text-left">
                                                <thead
                                                    className="bg-[#f7f9fe] text-[11px] font-bold uppercase tracking-[0.04em] text-[#6b7280] xl:text-xs">
                                                <tr>
                                                    ...
                                                    selectedOccupants.map((occupant, index) => (
                                                        ...
                                                        >
                                                            <td data-label="Họ tên" className="px-4 py-3">
                                                                <p className="truncate font-bold text-[#091426]"
                                                                   title={occupant.fullName || "Chưa cập nhật"}>
                                                                    {occupant.fullName || "Chưa cập nhật"}
                                                                </p>
                                                            </td>
                                                            <td data-label="Vai trò" className="px-4 py-3">
                              <span
                                  className="inline-flex max-w-full rounded-full border border-[#d8e1f2] bg-[#f8fbff] px-2.5 py-1 text-[11px] font-bold text-[#34445c] xl:text-xs">
                                <span className="truncate">
                                  {ROLE_LABELS[occupant.occupantRole] || occupant.occupantRole || "Chưa rõ"}
                                                            ...
                                                            <td data-label="Ý định" className="px-4 py-3">
                                                                <span
                                                                    ...
                                                                    title={occupant.occupantIntentionNote || undefined}
                                                                >
                                                                    <span className="truncate">
                                                                        {OCCUPANT_INTENTION_LABELS[occupant.occupantIntention] || "Chưa phản hồi"}
                                                                    </span>
                                                                </span>
                                                            </td>
                                                            <td data-label="SĐT"
                                                            ...
                                                            <td data-label="CCCD"
                                                                className="break-words px-4 py-3 text-[#4b5563]"
                                                                title={formatIdentityNumber(occupant.citizenId || occupant.identityNumber)}>
                                        ...
                                        <div className="mt-5 grid gap-4 sm:grid-cols-2 xl:gap-5">
                                            <InfoValue label="Tổng số người"
                                                       value={`${getOccupantsCount(mergedSelected, details)} người`}/>
                                            <InfoValue label="Giá thuê"
                                    ...
                                    {shouldShowSignedFileCard(mergedSelected) && (
                                        <DetailCard title="File hợp đồng đã ký" icon={FileCheck2}
                                                    className="lg:col-span-2">
                                            <div className="mt-5 rounded-lg bg-white p-4">
                                                {getLeaseSignedFileId(mergedSelected) ? (
                                                    ...
                                                    <div
                                                        className="rounded-lg border border-dashed border-red-300 bg-white p-5 text-center">
                                                        <Upload className="mx-auto h-8 w-8 text-red-500"/>
                                                        <p className="mt-2 font-extrabold text-[#091426]">
                                                            Chưa có file hợp đồng cho
                                                            phòng {mergedSelected.roomCode || "chưa rõ"}
                                                        </p>
                                                        <p className="mt-1 text-sm text-[#607089]">
                                                            Khách: {mergedSelected.primaryTenantName || mergedSelected.customerName || "Chưa có"} -
                                    ...
                                    {mergedSelected.leaseContractId && (
                                        <DetailCard title="Nguyện vọng khách thuê" icon={FileWarning}
                                                    className="lg:col-span-2">
                                            <div className="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
                                                ...
                                                />
                                                <InfoValue label="Nguồn ghi nhận"
                                                           value={tenantIntentionInfo.sourceLabel}/>
                                                <InfoValue label="Cập nhật lần cuối"
                                                           value={formatDateTime(tenantIntentionInfo.recordedAt)}/>
                                            </div>
                                            <div
                                                className="mt-4 rounded-xl border border-[#dfe5ef] bg-[#f8fafc] px-4 py-3">
                                                <p className="text-xs font-bold uppercase tracking-[0.08em] text-[#718096]">Lý
                                                    do / ghi chú</p>
                                            ...
                                            <p className="mt-3 text-xs font-semibold leading-5 text-[#607089]">
                                                Dữ liệu này được đọc trực tiếp từ backend. Nút bên dưới chỉ dùng khi
                                                quản lý ghi nhận thay khách hoặc cập nhật lại sau khi trao đổi trực
                                                tiếp.
                                            </p>
                                        </DetailCard>
                                    )}
                                    ...
                                    <section className="grid gap-3 lg:col-span-2 sm:grid-cols-2">
                                        ...
                                        {getWorkflow(mergedSelected) === "RENEWED" && (
                                            <p className="flex min-h-11 items-center rounded-lg border border-blue-200 bg-blue-50 px-4 text-sm font-semibold text-blue-700 sm:col-span-2">
                                                Hợp đồng đã gia hạn. Tài khoản khách thuê được sử dụng tiếp ở hợp đồng
                                                mới.
                                            </p>
                                        )}
                                        {(details?.canRenew ??
                ...
                <div
                    ...
                    <section className="max-h-[92vh] w-full max-w-3xl overflow-y-auto rounded-2xl bg-white shadow-2xl"
                        ...
                        <header className="flex items-start justify-between gap-4 border-b border-[#dfe5ef] px-5 py-5">
                            <div>
                                <h2 className="text-xl font-extrabold text-[#091426]">Gia hạn hợp đồng</h2>
                                <p className="mt-1 text-sm text-[#607089]">
                                    Cập nhật thời hạn và điều khoản trên hợp đồng {mergedSelected.contractCode}. Không tạo hợp đồng mới.
                                </p>
                            </div>
                            <button
                                type="button"
                        ...
                        <div className="grid gap-4 px-5 py-5 sm:grid-cols-2">
                            <div className="rounded-xl border border-[#dfe5ef] bg-[#f8fafc] p-4 sm:col-span-2">
                                <p className="text-sm font-extrabold text-[#091426]">Hợp đồng hiện tại</p>
                                <div className="mt-3 grid gap-3 sm:grid-cols-2">
                                    <InfoValue label="Ngày bắt đầu"
                                               value={formatDisplayDate(renewForm.startDate) || "Chưa cập nhật"}/>
                                    <InfoValue label="Ngày kết thúc hiện tại"
                                               value={formatDisplayDate(renewForm.currentEndDate) || "Chưa cập nhật"}/>
                                </div>
                            ...
                            <label className="grid gap-1.5">
                                <span className="text-sm font-bold text-[#34445c]">Ngày kết thúc sau gia hạn *</span>
                                <DateInput
                                    value={renewForm.endDate}
                                    min={renewForm.currentEndDate ? addDays(renewForm.currentEndDate, 1) : undefined}
                                    onChange={(event) => updateRenewField("endDate", event.target.value)}
                                ...
                                {renewFieldErrors.endDate && (
                                    <span className="text-xs font-semibold text-red-600">{renewFieldErrors.endDate}</span>
                                )}
                            </label>
                            ...
                            {[
                                ...
                                <label key={field} className="grid gap-1.5">
                                    ...
                                    <input
                                        ...
                                        className={`h-11 rounded-lg border px-3 text-sm font-semibold outline-none ${renewFieldErrors[field] ? "border-red-500" : "border-[#cbd5e1] focus:border-[#091426]"
                                        }`}
                                    />
                                    {renewFieldErrors[field] && (
                                        <span
                                            className="text-xs font-semibold text-red-600">{renewFieldErrors[field]}</span>
                                    )}
                                </label>
                            ))}
                            ...
                            <label className="grid gap-1.5">
                                ...
                                {renewFieldErrors.paymentCycleMonths && (
                                    <span
                                        className="text-xs font-semibold text-red-600">{renewFieldErrors.paymentCycleMonths}</span>
                                )}
                            </label>

                            <div className="rounded-xl border border-[#dfe5ef] bg-[#f8fafc] p-4 sm:col-span-2">
                                <p className="text-sm font-extrabold text-[#091426]">Người ở giữ nguyên trên hợp đồng hiện tại</p>
                                    ...
                                    {selectedOccupants.map((occupant, index) => (
                                        ...
                                        >
                                            ...
                                            <p className="mt-1 text-xs font-semibold text-[#34445c]">
                                                {OCCUPANT_INTENTION_LABELS[occupant.occupantIntention] || "Chưa phản hồi ý định"}
                                            </p>
                                        </div>
                                    ))}
                                </div>
                            ...
                            {renewError && (
                                <div
                                    className="rounded-lg border border-red-200 dark:border-rose-500/20 bg-red-50 dark:bg-rose-500/10 px-3 py-2 text-sm font-bold text-red-700 dark:text-rose-300 sm:col-span-2">
                                    {renewError}
                                </div>
                            )}
                        </div>
                        ...
                        <footer
                            className="flex justify-end gap-3 border-t border-[#dfe5ef] dark:border-white/10 px-5 py-4">
                            <button
                                type="button"
                                onClick={() => setRenewModalOpen(false)}
            ...
            {intentionModalOpen && mergedSelected && (
                <div
                    ...
                    <section className="w-full max-w-lg rounded-2xl bg-white shadow-2xl"
                        ...
                        <header className="flex items-start justify-between border-b border-[#dfe5ef] px-5 py-5">
                            <div>
                                <h2 className="text-xl font-extrabold text-[#091426]">Ghi nhận / Cập nhật ý định
                                    khách</h2>
                ...
                <span className="text-sm font-bold text-slate-700 dark:text-slate-200">
                  Ý định
                            ...
                            {["MOVE_OUT", "TRANSFER"].includes(intentionForm.intention) && (
                                <label className="grid gap-1.5">
                                    ...
                                    <input
                                        ...
                                        onChange={(event) => setIntentionForm((current) => ({
                                            ...current,
                            ...
                            {intentionError && (
                                <div
                                    className="rounded-lg border border-red-200 dark:border-rose-500/20 bg-red-50 dark:bg-rose-500/10 px-3 py-2 text-sm font-bold text-red-700 dark:text-rose-300">
                                    {intentionError}
                                </div>
                            )}
                        </div>
