...
function getAgreementPagination(response, fallback = {}) {
  const items = getAgreementItems(response);
  const totalElements = Number(
    response?.totalElements ?? response?.total_elements ?? items.length,
  );
  const totalPages = Number(
    response?.totalPages ??
      response?.total_pages ??
      Math.ceil(totalElements / Math.max(1, fallback.size || 10)),
  );
...
export default function DepositsPage() {
  ...
  const loadAgreements = useCallback(async () => {
    try {
      ...
      const pagination = getAgreementPagination(response, { size });
      setAgreements(sortByNewest(nextAgreements, ["createdAt", "created_at"]));
  ...
  }, [customerFilter, floorFilter, page, size, statusFilter]);
  ...
  const filteredTotalElements = totalElements;
  const filteredTotalPages = totalPages;
  const pagedAgreements = filteredAgreements;
