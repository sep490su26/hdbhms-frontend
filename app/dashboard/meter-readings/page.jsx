...
function NavIcon({type, className = "w-5 h-5"}) {
    ...
    const icons = {
        ...
        dollar: <>
            ...
            <path d="M12 6v12M9 9h4.5a1.5 1.5 0 010 3H10.5a1.5 1.5 0 000 3H15" {...s} />
        </>,
        "alert-circle": <>
            <circle cx="12" cy="12" r="10" {...s} />
        ...
        tool: <>
            <path
                d="M14.7 6.3a1 1 0 000 1.4l1.6 1.6a1 1 0 001.4 0l3.77-3.77a6 6 0 01-7.94 7.94l-6.91 6.91a2.12 2.12 0 01-3-3l6.91-6.91a6 6 0 017.94-7.94l-3.76 3.76z" {...s} />
        </>,
        clipboard: <>
        ...
        headphones: <>
            ...
            <path
                d="M21 19a2 2 0 01-2 2h-1a2 2 0 01-2-2v-3a2 2 0 012-2h3zM3 19a2 2 0 002 2h1a2 2 0 002-2v-3a2 2 0 00-2-2H3z" {...s} />
        </>,
    };
...
export default function UtilityManagement() {
            ...
            try {
                ...
                if (dashboardRes) {
                    ...
                    setDashboard({
                        ...
                        currentPeriod: dashboardRes.currentPeriod ?? dashboardRes.current_period
                    });
    ...
    const currentStatus = currentPeriod ? STATUS_MAP[currentPeriod.status] || STATUS_MAP.DRAFT : STATUS_MAP.DRAFT;
    ...
    const progress = totalRooms === 0 ? 0 : Math.round((completedRooms / totalRooms) * 100);
        ...
        <div className="w-full min-w-0 overflow-x-hidden bg-gray-50 font-sans">
            <div className="flex w-full min-w-0 flex-col gap-6">
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
                        <div>
                            <h1 className="text-2xl font-bold text-gray-900">Nhập điện nước hàng tháng</h1>
                            <p className="text-sm text-gray-400 mt-0.5">Quản lý ghi chỉ số điện nước hàng tháng</p>
                        </div>
                    ...
                    <div className="bg-white border border-gray-200 rounded-2xl p-5 mb-4">
                        {!currentPeriod ? (
                            ...
                            <div className="flex flex-col md:flex-row items-stretch gap-6">
                                {/* Period info */}
                                <div
                                    className="w-full md:w-44 shrink-0 md:border-r border-b md:border-b-0 border-gray-100 md:pr-6 pb-4 md:pb-0">
                                    <p className="text-xs text-gray-400 mb-1">Kỳ ghi chỉ số hiện tại</p>
                                    <p className="text-3xl font-bold text-blue-600 mb-2">{currentPeriod?.period}</p>
                                    <span
                                ...
                                {/* Progress */}
                                <div
                                    className="flex-1 md:border-r border-b md:border-b-0 border-gray-100 md:pr-6 pb-4 md:pb-0">
                                    <p className="text-xs text-gray-400 mb-3">Tiến độ nhập chỉ số</p>
                                    ...
                                    <p className="text-sm text-gray-500">{completedRooms} / {totalRooms} phòng</p>
                                </div>
                                ...
                                <div className="flex items-center justify-between md:justify-start gap-4 md:gap-6">
                                    <div className="text-center">
                                        ...
                                        <p className="text-xs text-gray-400">Đã nhập</p>
                                    </div>
                                    <div className="text-center">
                                        <div
                                    ...
                                    <div className="text-center">
                                        ...
                                        <p className="text-xs text-gray-400">Chưa nhập</p>
                                    </div>
                                </div>
                            </div>
                        )}
                    ...
                    <div className="bg-white border border-gray-200 rounded-2xl overflow-hidden mb-5">
                        <div className="px-5 py-4 border-b border-gray-100">
                            <h2 className="font-semibold text-gray-800">Lịch sử các kỳ ghi chỉ số</h2>
                        </div>
                                ...
                                <Table className="min-w-[800px]">
                                    <TableHeader>
                                        <TableRow className="bg-gray-50 hover:bg-gray-50 border-b border-gray-100">
                                            <TableHead
                                                className="text-xs font-semibold text-gray-400 uppercase tracking-wide">Kỳ
                                                ghi chỉ số</TableHead>
                                            <TableHead
                                                className="text-xs font-semibold text-gray-400 uppercase tracking-wide">Thời
                                                gian</TableHead>
                                            <TableHead
                                                className="text-xs font-semibold text-gray-400 uppercase tracking-wide">Trạng
                                                thái</TableHead>
                                            <TableHead
                                                className="text-xs font-semibold text-gray-400 uppercase tracking-wide">Tiến
                                                độ</TableHead>
                                            <TableHead
                                                className="text-xs font-semibold text-gray-400 uppercase tracking-wide">Phòng
                                                đã nhập</TableHead>
                                            <TableHead
                                                className="text-xs font-semibold text-gray-400 uppercase tracking-wide">Cảnh
                                                báo</TableHead>
                                            <TableHead
                                                className="text-xs font-semibold text-gray-400 uppercase tracking-wide">Thao
                                                tác</TableHead>
                                        </TableRow>
                                    </TableHeader>
                                    <TableBody>
                                        {paginatedHistory.map((h) => {
                                            ...
                                            return (
                                                <TableRow key={h.period}
                                                    ...
                                                    <TableCell className="py-3.5">
                                                        {h.isCurrent ? (
                                                            <span className="font-semibold text-blue-600">{h.period}
                                                                <span className="text-xs">(Hiện tại)</span></span>
                                                        ) : (
                                                            <span
                                                                className="font-medium text-gray-800">{h.period}</span>
                                                        )}
                                                    </TableCell>
                                                    <TableCell
                                                        className="py-3.5 text-gray-500">{formatTime(h.startDate, h.endDate)}</TableCell>
                                                    <TableCell className="py-3.5">
                                                        <span
                                                            className={`inline-block text-xs font-semibold px-2.5 py-1 rounded-lg ${st.bg}`}>{st.label}</span>
                                                    </TableCell>
                                                    <TableCell className="py-3.5">
                                                    ...
                                                    <TableCell
                                                        className="py-3.5 text-gray-600">{h.completedRooms} / {h.totalRooms}</TableCell>
                                                    <TableCell className="py-3.5">
                                                        <span
                                                            className={`text-sm font-semibold ${h.anomalyCount > 0 ? "text-orange-500" : "text-gray-400"}`}>{h.anomalyCount}</span>
                                                    </TableCell>
                                                    <TableCell className="py-3.5">
                                                        <div className="flex items-center gap-2">
                                                            <button
                                            ...
                                            <div className="space-y-3 mb-4">
                                                <div>
                                                    <div className="flex justify-between items-center mb-1 text-sm">
                                                        <span
                                                            className="text-gray-500">Tiến độ ({h.completedRooms} / {h.totalRooms})</span>
                                                ...
                                                <div className="flex justify-between items-center text-sm">
                                                    ...
                                                    <span
                                                        className={`font-semibold ${h.anomalyCount > 0 ? "text-orange-500" : "text-gray-700"}`}>{h.anomalyCount}</span>
                                                </div>
                                            </div>

                                            <button
                                                onClick={() => router.push(`/dashboard/meter-readings/batch?period=${h.period}`)}
                                                className="w-full text-sm font-medium text-blue-600 bg-white hover:bg-blue-50 border border-blue-200 py-2 rounded-lg transition-colors">
                                                Xem chi tiết
                                            </button>
                                        </div>
                                    );
                        ...
                        <div className="bg-white border border-gray-200 rounded-2xl p-5">
                            <div className="flex items-center gap-2 mb-4">
                                ...
                                <div>
                                    ...
                                    <p className="text-xs text-gray-400">Các bước ghi chỉ số điện nước hàng tháng</p>
                                </div>
                            </div>
                                ...
                                {[
                                    ...
                                    <div key={step.num} className="flex items-center gap-1 flex-1">
                                        <div className="flex flex-col items-center text-center flex-1">
                                            <div
                                                className="w-10 h-10 bg-blue-50 border border-blue-100 rounded-xl flex items-center justify-center text-lg mb-1.5">{step.icon}</div>
                                            <p className="text-[11px] text-gray-500 leading-tight">{step.num}. {step.label}</p>
                                        </div>
                                        {i < 3 && (
