...
function RoomDetailPanel({room, tenantList, onClose}) {
    ...
    return (
        <aside
            className="flex h-full w-[320px] shrink-0 flex-col overflow-hidden rounded-xl border border-[#e2e8f0] bg-white shadow-lg">
            <div className="flex items-start justify-between border-b border-[#e2e8f0] bg-[#091426] px-5 py-4">
                <div>
                    <div className="flex items-center gap-2">
                        ...
                        <span
                            className={`inline-flex rounded-full px-2.5 py-0.5 text-[10px] font-bold ring-1 ring-inset ${statusCls}`}>
              {statusLabel}
            </span>
                    </div>
            ...
            <div className="flex-1 overflow-y-auto p-5 custom-scrollbar">
                        ...
                        {isDetailLoading ? (
                            ...
                            <>
                                <div className="rounded-lg border border-[#e2e8f0] p-4">
                                    <p className="text-[10px] font-bold uppercase tracking-wider text-[#45474c]">Giá
                                        thuê</p>
                                ...
                                <div className="grid grid-cols-2 gap-2">
                                    {[
                                        ...
                                        {label: "Đặc điểm", value: roomDetail?.publicNote ?? "Không có"},
                                        {
                                            ...
                                            value: roomDetail?.deletedAt ? "Đã xóa mềm" : "Hoạt động"
                                        },
                                    ].map(({label, value}) => (
                                ...
                                <div className="rounded-lg border border-amber-100 bg-amber-50 p-3">
                                    <p className="text-[10px] font-semibold uppercase text-amber-800">Ghi chú nội bộ
                                        (Staff Only)</p>
                                    <p className="mt-0.5 text-sm text-amber-900">{roomDetail?.internalNote ?? "Không có ghi chú"}</p>
                                </div>

                                <div className="mt-4">
                                    <label className="text-[10px] font-bold uppercase tracking-wider text-[#45474c]">Mã
                                        phòng (Cập nhật)</label>
                                    <input
                                        type="text"
                ...
                {detailTab === "tenant" && (
                  ...
                  <span
                      className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-[#091426] text-sm font-bold text-white">
                    {tenant.initials}
                  </span>
                                    <div>
                                ...
                                {[
                                    ...
                                    <div key={label}
                                         className="flex items-start gap-3 rounded-lg bg-[#f7f9fb] px-4 py-3">
...
function FloorMapPage({tenantList = []}) {
    ...
    useEffect(() => {
        const fetchStaffRooms = async () => {
            try {
                setIsLoading(true);
                const data = await authenticatedFetch(`/rooms?size=100`);
                setApiRooms(data.data?.content ?? []);
                setIsSuccess(true);
            } catch (error) {
                setIsError(true);
            } finally {
                ...
                <div className="mt-6 space-y-6">
                    <section className="grid grid-cols-2 gap-3 sm:grid-cols-4">
                        {[
                            ...
                            <article key={label}
                                     className={`flex flex-col rounded-xl border border-[#e2e8f0] ${bg} px-5 py-4 shadow-[0_1px_2px_rgba(9,20,38,0.06)]`}>
                    ...
                    <div
                        ...
                        <div
                            className="flex items-center justify-between gap-4 border-b border-[#e2e8f0] bg-[#f7f9fb] px-5 py-3">
                            ...
                            <div className="hidden items-center gap-4 xl:flex">
                                {FLOOR_STATUSES.map(({key, label, dot}) => (
                                    <span key={key}
                                          className="flex items-center gap-1.5 text-[11px] font-semibold text-[#45474c]">
                    <span className={`h-2 w-2 rounded-full ${dot}`}/>
                        ...
                        <div
                            className="flex flex-wrap items-center justify-center gap-5 border-b border-[#e2e8f0] bg-[#091426]/[0.03] px-5 py-3">
                            <span
                                className="text-[10px] font-bold uppercase tracking-widest text-[#6b7280]">Màu ô:</span>
                            {[
                            ...
                            <div className="flex-1 p-5">
                                <div className="mx-auto grid max-w-[920px] grid-cols-1 gap-5 lg:grid-cols-[214px_1fr]">
                                    <div className="grid content-start gap-3">
                                        <p className="text-[10px] font-bold uppercase tracking-widest text-[#6b7280]">Phòng
                                            lớn</p>
                                        {leftRooms.map((room) => (
                                            <RoomCell
                                                key={room.id}
                                        ...
                                        <div
                                            className="flex min-h-[62px] items-center justify-center rounded-xl border border-dashed border-slate-200 bg-slate-50 py-4">
                                            <span
                                                className="text-[10px] font-semibold uppercase tracking-widest text-slate-400">Cầu thang</span>
                                        </div>
                                    </div>
                      ...
                      <span className={`h-2 w-2 rounded-full ${dot}`}/>
                            ...
                            {selectedRoom && (
                                <>
                                    ...
                                    />
                                    <div
                                        className="fixed inset-y-0 right-0 z-40 flex w-[340px] max-w-[90vw] flex-col bg-[#f7f9fb] p-3 shadow-2xl xl:static xl:z-0 xl:w-auto xl:shrink-0 xl:border-l xl:border-[#e2e8f0] xl:bg-transparent xl:p-3 xl:shadow-none">
                                        <RoomDetailPanel
                                            room={selectedRoom}
