    ...
    return (
        ...
        >
            ...
            {preview ? (
                ...
                <span
                    className="flex h-12 w-12 items-center justify-center rounded-full bg-[#eef2ff] text-[#232946] transition group-hover:bg-[#e0e7ff]">
...
function DepositInfoForm({room, onSubmit, isSubmitting, blockingStatus}) {
    ...
    const handleSubmit = (event) => {
        ...
        const depositMonths = parseInt(data.depositMonths, 10);
        const paymentCycleMonths = parseInt(data.paymentCycleMonths, 10);

        if (isNaN(depositMonths) || depositMonths < 1) {
            nextErrors.depositMonths = "Vui lòng nhập số tháng đặt cọc hợp lệ (từ 1 trở lên).";
        }
        if (isNaN(paymentCycleMonths) || !(paymentCycleMonths === 1 || paymentCycleMonths === 3)) {
            nextErrors.paymentCycleMonths = "Chu kỳ thanh toán chỉ chấp nhận 1 hoặc 3 tháng.";
        }
        ...
        const metadata = {
            ...
            deposit_months: parseInt(data.depositMonths, 10),
            payment_cycle_months: parseInt(data.paymentCycleMonths, 10)
        };
    ...
    return (
        <DepositFormErrorContext.Provider value={{errors: fieldErrors, setError: setFieldError}}>
            <section
                className="rounded-xl border border-[#c5c6cd] bg-[#fbf8fa] p-6 shadow-[0_4px_10px_rgba(9,20,38,0.04)] sm:p-8">
                <div>
                ...
                <form onSubmit={handleSubmit} noValidate className="mt-8 grid gap-x-6 gap-y-6 sm:grid-cols-2">
                    ...
                    <Field label="Email (không bắt buộc)" name="email" type="email" placeholder="example@gmail.com"
                           required={false}/>
                    ...
                    <Field className="sm:col-span-2" label="Địa chỉ thường trú" name="permanentAddress"
            ...
            Tôi cam kết các thông tin trên là chính xác và đồng ý với các <strong className="text-[#091426]">điều khoản đặt cọc</strong> của Hải Đăng House.
                    ...
                    {Object.values(fieldErrors).some(Boolean) && (
                        <div
                            className="rounded-lg border border-rose-200 bg-rose-50 px-4 py-3 text-sm font-medium text-rose-700 sm:col-span-2">
...
function DepositPaymentStep({room, customer, paymentIntent}) {
    ...
    if (isConfirmed) {
        return (
            <section
                className="flex flex-col items-center justify-center rounded-xl border border-[#c5c6cd] bg-[#fbf8fa] p-10 text-center shadow-[0_4px_10px_rgba(9,20,38,0.04)]">
                ...
                <p className="mt-2 max-w-md text-sm leading-6 text-[#45474c]">
                    Yêu cầu đặt cọc phòng {room.id} đã được ghi nhận. Chủ nhà sẽ liên hệ xác nhận trong thời gian sớm
                    nhất.
                </p>
                <Link
                    href="/rooms"
    ...
    return (
        <section
            className="rounded-xl border border-[#c5c6cd] bg-[#fbf8fa] p-6 shadow-[0_4px_10px_rgba(9,20,38,0.04)] sm:p-8">
            <div
                className="flex flex-col gap-4 border-b border-[#c5c6cd] pb-6 md:flex-row md:items-start md:justify-between">
                <div>
            ...
            <div className="mt-8 grid gap-6 lg:grid-cols-[1fr_280px]">
                ...
                <div className="rounded-xl border border-[#c5c6cd] bg-white p-5 text-center">
                    <div
                        className="mx-auto flex aspect-square w-full max-w-[220px] items-center justify-center rounded-xl border border-dashed border-[#c5c6cd] bg-[#f5f3f4]">
                        <div className="grid h-32 w-32 grid-cols-5 grid-rows-5 gap-1">
                            {Array.from({length: 25}).map((_, index) => (
                                <span key={index}
                                      className={`${index % 2 === 0 || index % 7 === 0 ? "bg-[#091426]" : "bg-white"} rounded-sm`}/>
                            ))}
                        </div>
                    </div>
                    <p className="mt-4 text-sm leading-6 text-[#45474c]">Mã QR minh họa. Khi kết nối cổng thanh toán, hệ
                        thống sẽ sinh QR theo mã {paymentCode}.</p>
                    <div className="mt-4 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3">
                        <p className="text-xs font-bold uppercase tracking-[0.08em] text-amber-700">Thời gian giữ chỗ
                            còn lại</p>
                        <p className="mt-1 text-2xl font-bold text-amber-800">{formatHoldCountdown(remainingMs)}</p>
                        <p className="mt-1 text-xs leading-5 text-amber-700">Hết thời gian, hệ thống sẽ trả phòng về
                            trạng thái trống.</p>
                    </div>
                </div>
            </div>
        ...
        <div className="min-h-screen bg-[#fbf8fa] px-4 pb-20 pt-8 text-[#091426] sm:px-6 lg:px-12">
            <div className="mx-auto max-w-[1120px]">
                ...
                <div className="grid gap-8 lg:grid-cols-[352px_1fr]">
                    ...
                    {step === "deposit" &&
                        <DepositPaymentStep room={room} customer={customer} paymentIntent={paymentIntent}/>}
                </div>
