...
export async function fetchPublicRoomById(roomIdentifier) {
  ...
  // If the identifier is a pure number, prepend "P" to match room codes like P101, P104, etc.
  const normalizedIdentifier = /^\d+$/.test(roomIdentifier) ? `P${roomIdentifier}` : roomIdentifier;


  try {

    const response = await fetch(`${PUBLIC_ROOMS_API_URL}/${encodeURIComponent(normalizedIdentifier)}`, { cache: "no-store" });
...
export function getRoomDetailHref(room) {
  ...
  // Check if buildingId is a numeric ID, otherwise use default

  const safeBuildingId = /^\d+$/.test(buildingId) ? buildingId : "hai-dang-house";

  

  return `/rooms/${safeBuildingId}/${roomId}`;
